...
class ClientUnlearnGym(UnlearnGym):
    def __init__(self, global_model: nn.Module, n_clients: int, delta: Optional[float], tau: float, *args, **kwargs):
    ...
    def project_norm_ball(self):
        for ref_parameter, client_parameter, delta in zip(self.ref_model.parameters(), self.model.parameters(),
                                                          self.deltas):
...
class FederatedUnlearnGym(FederatedGym):
    def __init__(self,
                 ...
                 tau: float, n_clients: int, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def untrain(self,
        ...
        unfed_gym = ClientUnlearnGym(train_loader=self.unclient_train_loader, val_loader=self.unclient_val_loader,
                                     ...
                                     metric=self.metric,
                                     delta=self.delta, tau=self.tau, n_clients=self.n_clients)
